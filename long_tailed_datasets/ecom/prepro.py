...
def process():
    ...
    test_examples = []
    label2idx = {}
    with open("train_0.json", "r") as fi:
        for i, line in enumerate(fi):
            ...
            train_examples.append((text_a, label))
    with open("dev_0.json", "r") as fi:
        for i, line in enumerate(fi):
            d = json.loads(line.strip())
            text_a = d["sentence"]
            label = d["label"]
            if label not in label2idx:
                label2idx[label] = len(label2idx)
            dev_examples.append((text_a, label))
    with open("test_public.json", "r") as fi:
        for i, line in enumerate(fi):
            d = json.loads(line.strip())
            text_a = d["sentence"]
            label = d["label"]
            if label not in label2idx:
                label2idx[label] = len(label2idx)
            test_examples.append((text_a, label))
